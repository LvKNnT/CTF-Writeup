import hashlib
from Crypto.Cipher import AES
from Crypto.Util.Padding import unpad

# --- 1. Load Parameters ---
# Curve params from source 
p = 0xa15c4fb663a578d8b2496d3151a946119ee42695e18e13e90600192b1d0abdbb6f787f90c8d102ff88e284dd4526f5f6b6c980bf88f1d0490714b67e8a2a2b77
a = 0x5e009506fcc7eff573bc960d88638fe25e76a9b6c7caeea072a27dcd1fa46abb15b7b6210cf90caba982893ee2779669bac06e267013486b22ff3e24abae2d42
b = 0x2ce7d1ca4493b0977f088f6d30d9241f8048fdea112cc385b793bce953998caae680864a7d3aa437ea3ffd1441ca3fb352b0b710bb3f053e980e503be9a7fece

# Define Curve
E = EllipticCurve(GF(p), [a, b])

# Points from output 
# Generator G
G_x = 3034712809375537908102988750113382444008758539448972750581525810900634243392172703684905257490982543775233630011707375189041302436945106395617312498769005
G_y = 4986645098582616415690074082237817624424333339074969364527548107042876175480894132576399611027847402879885574130125050842710052291870268101817275410204850
G = E(G_x, G_y)

# Alice's Public Key A
A_x = 4748198372895404866752111766626421927481971519483471383813044005699388317650395315193922226704604937454742608233124831870493636003725200307683939875286865
A_y = 2421873309002279841021791369884483308051497215798017509805302041102468310636822060707350789776065212606890489706597369526562336256272258544226688832663757
A = E(A_x, A_y)

# Bob's Public Key B (hardcoded in source) 
b_x = 0x7f0489e4efe6905f039476db54f9b6eac654c780342169155344abc5ac90167adc6b8dabacec643cbe420abffe9760cbc3e8a2b508d24779461c19b20e242a38
b_y = 0xdd04134e747354e5b9618d8cb3f60e03a74a709d4956641b234daa8a65d43df34e18d00a59c070801178d198e8905ef670118c15b0906d3a00a662d3a2736bf
B = E(b_x, b_y)

# Ciphertext data 
iv_hex = '719700b2470525781cc844db1febd994'
ct_hex = '335470f413c225b705db2e930b9d460d3947b3836059fb890b044e46cbb343f0'

# --- 2. Smart's Attack Implementation ---

print(f"Checking for Anomalous Curve...")
if E.order() == p:
    print(f"Confirmed: Curve order equals p. Vulnerable to Smart's Attack.")
else:
    print("Warning: Curve order != p. Smart's attack might not work.")

# Lift to p-adic numbers with precision 2
E_Qp = EllipticCurve(Qp(p, 2), [a, b])

def smart_log(P):
    """
    Computes the p-adic elliptic logarithm.
    P must be a point on E(GF(p)).
    We lift P to E(Qp), multiply by p (to map into the formal group kernel),
    and then compute the isomorphism to the additive group.
    """
    # Lift the point to Qp
    P_lifted = E_Qp.lift_x(Integer(P[0]), all=True)[0] 
    
    # Check y-coordinate to ensure we picked the right lift (sign matches)
    # The lift_x might return y or -y. We compare modulo p.
    py = P_lifted.xy()[1]
    if Integer(py) % p != Integer(P[1]):
        P_lifted = -P_lifted
        
    # Multiply by p to project into the kernel of reduction
    P_kernel = p * P_lifted
    x_k, y_k = P_kernel.xy()
    
    # The logarithm is -x/y in the formal group
    return -(x_k / y_k)

# Compute logs
print("Calculating discrete log of A base G...")
log_G = smart_log(G)
log_A = smart_log(A)

# Calculate private key nA = log(A) / log(G) mod p
nA = Integer(log_A / log_G)
print(f"Found Private Key nA: {nA}")

# --- 3. Decrypt ---
print("Decrypting...")

# Calculate Shared Secret 
S = nA * B
secret = S.xy()[0]

# Derive AES Key 
sha1 = hashlib.sha1()
sha1.update(str(secret).encode('ascii'))
key = sha1.digest()[:16]

# Decrypt AES 
iv = bytes.fromhex(iv_hex)
ciphertext = bytes.fromhex(ct_hex)
cipher = AES.new(key, AES.MODE_CBC, iv)
try:
    plaintext = unpad(cipher.decrypt(ciphertext), 16)
    print(f"\nFLAG: {plaintext.decode()}")
except Exception as e:
    print(f"Decryption failed: {e}")