...
_sage_const_13413 = Integer(13413); _sage_const_64 = Integer(64); _sage_const_257 = Integer(257); _sage_const_1048583 = Integer(1048583); _sage_const_80 = Integer(80); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_5 = Integer(5); _sage_const_256 = Integer(256)
from pwn import *
...
def solve():
    ...
    L = Matrix(ZZ, num_samples + n + _sage_const_1 , num_samples + _sage_const_1 )
    
    # Fill q*I (top left)
    L.set_block(_sage_const_0 , _sage_const_0 , q * identity_matrix(num_samples))
    
    # Fill A^T (middle left)
    L.set_block(num_samples, _sage_const_0 , A_mat.transpose())
    
    # Fill b' (bottom left)
    L[num_samples + n, :num_samples] = vector(ZZ, b_prime_list)
    
    # Fill 1 (bottom right)
    L[num_samples + n, num_samples] = _sage_const_1 
    ...
    for row in L_reduced:
        ...
        if row[num_samples] == _sage_const_1 :
            potential_e = row[:num_samples]
            # Verify it's small
            if all(abs(x) <= _sage_const_1  for x in potential_e):
                e_vector = potential_e
                break
        elif row[num_samples] == -_sage_const_1 :
            potential_e = -row[:num_samples]
            if all(abs(x) <= _sage_const_1  for x in potential_e):
                e_vector = potential_e
                break
    ...
    print(f"[+] Error vector recovered: {e_vector[:_sage_const_5 ]}...")
    ...
    while True:
        ...
        # Try e = 0, 1, -1
        for e_guess in [_sage_const_0 , _sage_const_1 , -_sage_const_1 ]:
            # m = diff - p*e
            m_candidate = (diff_int - p * e_guess) % q
            
            # Check if valid ascii/byte
            if _sage_const_0  <= m_candidate < _sage_const_256 :
