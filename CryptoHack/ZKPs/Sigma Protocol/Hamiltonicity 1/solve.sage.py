...
_sage_const_14635 = Integer(14635); _sage_const_0 = Integer(0); _sage_const_128 = Integer(128); _sage_const_5 = Integer(5); _sage_const_1 = Integer(1)
...
HOST, PORT = "archive.cryptohack.org", _sage_const_14635 
...
spec = importlib.util.spec_from_file_location(
    "hamiltonicity", glob.glob(os.path.join(HERE, "hamiltonicity*.py"))[_sage_const_0 ]
)
...
def gen_type1(FS_state):
    ...
    permutation = list(range(N))
    random.shuffle(permutation)
    cycle = [[permutation[i], permutation[(i + _sage_const_1 ) % N]] for i in range(N)]

    H = [[int(_sage_const_0 )] * N for _ in range(N)]
    for src, dst in cycle:
        H[src][dst] = int(_sage_const_1 )
...
for i in range(numrounds):
    while True:
        ...
        if state[-_sage_const_1 ] & _sage_const_1  == _sage_const_0 :
            break
        A, z, state = gen_type1(FS_state)
        if state[-_sage_const_1 ] & _sage_const_1  == _sage_const_1 :
            break

    FS_state = state
    io.recvuntil(b"send fiat shamir proof: ")
    io.sendline(json.dumps({"A": to_py(A), "z": to_py(z)}).encode())
    print(f"round {i}: bit {state[-_sage_const_1 ] & _sage_const_1 } -> {io.recvline().strip().decode()}")
