...
_sage_const_13431 = Integer(13431); _sage_const_2 = Integer(2); _sage_const_9 = Integer(9); _sage_const_0 = Integer(0); _sage_const_7 = Integer(7); _sage_const_8 = Integer(8); _sage_const_1 = Integer(1); _sage_const_39 = Integer(39)
...
def int_to_bytes(n, length=None):
    data = int(n).to_bytes((int(n).bit_length() + _sage_const_7 ) // _sage_const_8  or _sage_const_1 , "big")
...
def undo_xor_nonce(masked, nonce):
    return masked[:_sage_const_7 ] + xor(masked[_sage_const_7 :-_sage_const_1 ], nonce) + masked[-_sage_const_1 :]
...
def clean_flag(flag_with_junk):
    ...
    for i, b in enumerate(flag_with_junk):
        if b not in printable:
            candidate = flag_with_junk[:i] + flag_with_junk[i + _sage_const_1  :]
...
def recover_flag_int(records):
    ...
    hashes1 = [(salt, ZZ(challenge_hash(t1, y1, salt, g1))) for salt in range(_sage_const_2 , BITS + _sage_const_1 )]
    hashes2 = [(salt, ZZ(challenge_hash(t2, y2, salt, g2))) for salt in range(_sage_const_2 , BITS + _sage_const_1 )]
        ...
        for b2 in terms2:
            ...
            for salt1, c1 in hashes1:
                for salt2, c2 in hashes2:
                    delta_c = ZZ(c1 - c2)
                    if delta_c == _sage_const_0  or delta_b % delta_c != _sage_const_0 :
                        continue

                    x = ZZ(delta_b // delta_c)
                    if x <= _sage_const_0  or x.nbits() > _sage_const_8  * _sage_const_39 :
                        continue
...
def main():
    ...
    try:
        ...
        records = [get_known_prime_proof(io, nonce, KNOWN_SEEDS[_sage_const_0 ])]
        ...
        records.append(get_known_prime_proof(io, nonce, KNOWN_SEEDS[_sage_const_1 ]))

        hidden, salts = recover_flag_int(records)
        print(f"[+] recovered hash salts = {salts[_sage_const_0 ]}, {salts[_sage_const_1 ]}")

        masked = int_to_bytes(hidden, _sage_const_39 )
